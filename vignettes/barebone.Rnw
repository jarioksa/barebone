...
regression coefficients (which we did not need).

\index{QR decomposition}
Many numerical implementations of QR decomposition pivot the columns
...
ties in an order dependent way.
...
into same model matrix. Although the direct effect of conditions was
removed in the first step, they may interact with with \vec{X}.
Alteratively, we can use  residuals of \vec{X} regressed on \vec{Z},

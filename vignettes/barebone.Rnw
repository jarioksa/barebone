%\VignetteIndexEntry{barebone}
%\VignettePackage{barebone}
...
methods. This appendix has no traces of user friendliness.  The purpose is
...
\noalign{\smallskip}\svhline\noalign{\smallskip}
  ...
  && \texttt{crossprod(A, B)}\\
  Matrix outer product & \vec{AB'} & \texttt{A \%*\% t(B)} \\
  && \texttt{tcrossprod(A, B)} \\
   Identity matrix $5 \times 5$ & $\vec{I}_5$ & \texttt{diag(5)} \\
...
based on LAPACK and BLAS libraries \citep{LAPACK}. The
ordination methods can be very effectively programmed with these functions.
...
orthogonal, and their sum of squares is 1 and off-diagonal crossproduct elements
are 0, or $\vec{U'U} = \vec{I}$ and $\vec{V'V} = \vec{I}$. 
The importance of axes is conveyed in
...
reduced still further, and we speak of reduced rank or rank-deficit modes.

\index{singular value decomposition}
The original data can be recovered from the singular value decomposition The matrix equation
...
\index{singular value decomposition}
R function \icode{svd} performs the singular value decomposition. In
...
decomposition, is used for a symmetric matrix (in principle, spectral
...
nonsymmetric, and spectral decomposition is performed on their
...
eigenvalues sum up to total variance. If we use
 \texttt{scale(..., scale = TRUE)},
the columns will be centred and scaled by their standard deviations, and diagonal
elements of the crossproduct matrix will be 1 and off-diagonal elements
will be equal to  correlation coefficient \index{correlation coefficient}.  

\index{spectral decomposition}\index{eigenvector decomposition}
Function \icode{eigen} performs the spectral decomposition.  We can
...
\label{par:eigen-rank}
\index{spectral decomposition}\index{eigenvector decomposition}
The number of eigenvalues is given by the dimension of the symmetric
crossproduct matrix. If the number of rows was lower than then number
of columns in the original data, the rank of the crossproduct matrix is
lower than the number or columns and rows of the crossproducts.
For centred data of $n$ rows and $m$ columns the rank is $\min(m-1, n)$.
The \icode{eigen} function will still return $m$ eigenvalues and $m$ eigenvectors,
for the $m \times m$ crossproduct matrix, but only $n-1$ (rank) first of
these are meaningful and the rest are rubbish and should be ignored.

\subsection{QR Decomposition}
\label{sec:alg:QR}

\index{QR decomposition}
QR decomposition expresses any rectangular matrix \vec{X} as a product
of orthonormal matrix \vec{Q} and upper (or right) triangular matrix \vec{R} as
$\vec{X} = \vec{QR}$ (\vec{Q} and \vec{R} are amended with lines of
zeros for non-square \vec{X}). \vec{Q} is an orthogonal matrix,
which means that the transpose of $\vec{Q}$ is its
inverse $\vec{Q'} = \vec{Q}^{-1}$ and $\vec{Q'Q} = \vec{I}$.

QR decomposition is not directly used in
ordination, but it has an important position in linear models (regression) and
hence in constrained ordination \citep{BlueBook}. It should be seen
as a tool that is used and left behind.
...
intercept, and factors can be expressed as contrast or ``dummy´´ variables.
However, in ordination models we deal with centred data and no
intercept is needed. The fitted values $\vec{\hat Y}$ are found multiplying
predictors \vec{X} with regression coefficients \vec{B}.
\begin{eqnarray}
 \vec{\hat Y} &=& \vec{XB} \\
  \label{eq:Yhat}
...
function \icode{ginv} in MASS \citep{MASS}, but there is a real
danger of loss of numerical precision with nearly singular matrices.

\index{QR decomposition}

\index{QR decomposition}
Let us write eq. \ref{eq:Yhat} with QR decomposition of $\vec{X} = \vec{QR}$:
\begin{eqnarray}
 \vec{\hat Y} &=& \vec{QR}(\vec{R'\underbrace{Q'Q}_{\vec{I}} R})^{-1}\vec{R'Q'Y} \\
  \label{eq:QQtY}
&=& \vec{Q} \underbrace{\vec{R} \vec{R'}^{-1}}_{\vec{I}} \underbrace{\vec{R}^{-1} \vec{R'}}_{\vec{I}} \vec{Q'Y} \\
&=& \vec{QQ'Y}\;.
\end{eqnarray}
The hat matrix is $\vec{QQ'}$ and we avoided matrix inversion. For fitted values
$\vec{\hat Y}$ we did not need matrix \vec{R}, but we would need to see the
regression coefficients (which we did not need to get the fitted values).

\index{QR decomposition}
Many numerical implementations of QR decomposition pivot – or reorder – the columns
of model matrix (explanatory variables and factor contrasts) to improve
numerical stability. \index{aliased variables} The R implementation (function \icode{qr})
keeps the order of explanatory
variables except when some of the columns of model matrix are completely
redundant and can be expressed with the help of other columns. These 
redundant columns are moved to last ones, and the rank of the model matrix
is reduced. This is called pivoting. In Dutch Dune meadow data we have such
redundant variables: \texttt{Management} and \texttt{Manure} are not completely
independent because 0 \texttt{Manure} is only with Natural \texttt{Management}, and
\textsl{vice versa}. Function \texttt{qr} for  QR decomposition pivots this variable 
automatically

<<>>=
data(dune, dune.env)
X <- model.matrix(~ Manure + Management, data=dune.env)
Q <- qr(X)
str(Q)
@
Model matrix has eight columns, but the \texttt{rank} is 7, and the \texttt{pivot}
shows that column 7 was moved last beyond the first seven columns
given by the \texttt{rank}.

The use of \icode{qr} function is very simple. After making the QR decomposition
we can find fitted values, residuals and regression coefficients with

<<>>=
Yhat <- qr.fitted(Q, as.matrix(dune))
Yres <- qr.resid(Q, as.matrix(dune))
beta <- qr.coef(Q, as.matrix(dune))
@

\index{QR decomposition}
QR decomposition is a tool and inspecting matrices \vec{Q} or \vec{R} gives no
useful intuition of the data. It is best not to look at them. Actually, \icode{qr} function
even does not return those directly, but it only returns information that is need to find 
fitted values \textsl{etc}. If you really want to see the component matrices, they must
be calculated explicitly with functions \texttt{qr.Q} and \texttt{qr.R}.
...
eigenvector methods which can be easily solved with R matrix library and
nonmetric multidimensional scaling.

\subsection{Unconstrained Ordination}

Principal component analysis, Principal coordinate analysis and correspondence analysis are the major
eigenvector methods discussed in this chapter. These all are basically so similar that current vegan
uses the same shared code for all this (function \icode{ordConstrained}).

\subsubsection{Principal Component Analysis (PCA)}
\label{sec:alg:PCA}

\index{principal component analysis} \index{singular value decomposition}
...
orthonormal matrices of row and column scores.  This is implemented in
function \icode{PCA} which is based on R function \icode{svd}.

<<echo=false>>=
dump("PCA", file = "")
@
The function returns a result of the R function \icode{svd}. The only
...
column standard deviations, all columns will have equal weights.  The
...
$\sqrt{n}$ would give the same results with biased estimate of variance.

\paragraph{Spectral Decomposition}

\index{principal component analysis} \index{spectral decomposition}
...
\index{principal component analysis} \index{spectral decomposition}
The spectral composition method will attempt at finding as many eigenvalues
as there are data columns (variables). However, for $n$ observations only $n-1$
eigenvalues can be defined, and the rest are rubbish (see p.~\pageref{par:eigen-rank}).
The function \icode{PCAeig}  will
zero negative eigenvalues (similarly as \icode{princomp}), but will still return
rubbish scores beyond the $n-1$ axes (similarly as \icode{princomp} which however
will refuse the analysis if it knows that there are more columns than rows).
Function \icode{PCAeig} is based on R function \icode{eigen}.
...
Function \icode{PCAeig} used crossproduct \vec{\bar Y' \bar Y} to get
the symmetric matrix for spectral decomposition. Most PCA software use
instead covariance matrix. This is equivalent to finding the
...
that a random vector multiplied with a high power of a matrix will converge
to the first eigenvector of that matrix. The high power of matrix is
achieved by multiplying a vector several times with the matrix. We take a
...
multiplication is equivalent of finding weighted sums\index{weighted sums}, and therefore
...
iteratios when the eigenvalue stabilized in iterations.
Function \icode{PCApot1} implements this method:
...
\item \icode{prcomp} uses SVD, but it scales singular values by
  ...
  solution is found by scaling the response matrix with
  (\ref{eq:PCAstand}).
\index{principal component analysis}
\item \icode{princomp} uses eigen decomposition of covariance or
  ...
  \texttt{loadings}. Rectangular matrices are turned to covariance matrix,
  and if the number of rows is lower than the number of covers, \icode{prcomp} 
  will refuse the analysis. However, if supplied a covariance or a correlation
  matrix, it will analyse those cases, but eigenvalues and eigenvectors are
  rubbish beyond rank.
  \index{principal component analysis}
\item \icode{pca} and \icode{rda} in vegan package uses SVD in its PCA. The functions
  return singular values divided with $\sqrt{n-1}$ consistently with
...
\icode{PCAeig}).  The PCA with spectral decomposition analyses matrix
of crossproducts and sums of squares, but PCoA analyses distances.
It needs to convert distances into crossproducts,
and then analyse these like in \icode{PCAeig} \citep{Gower66,Mard79}. If
...
\begin{enumerate}
  \item Centre squared distances by rows and by columns
    ...
    presentation of centring, but in practice we use simpler process and just
    remove means. The example code below uses \texttt{scale}
  ...
  \item Matrix $-\tfrac{1}{2} \vec{M}$ presents the crossproducts and sum of
    squares and is submitted to spectral decomposition.
\end{enumerate}

Function \icode{PCoA} makes Gower centring followed by \icode{eigen}.
...
eigenvalues. If \vec{D} was non-Euclidean, there
...
eigenvalues are not removed, nor are any
warnings issued. If we wish to scale the row scores by eigenvalues, we
must use only the positive ones or absolute values.

\index{principal coordinate analysis}
This algorithm is used in standard R function \icode{cmdscale}
and vegan function \icode{wcmdscale}.
Functions like \icode{adonis2} and \icode{betadisper} in Vegan  use
...
\index{principal coordinate analysis!weighted}
Any eigenvector method can be weighted, but here we only demonstrate
weighting in PCoA.  Similar principles work with PCA or RDA
...
moment and they ``turn'' the ordination more strongly. 
Weighting moves points further from or closer to the
centroid.  After weijgting, we perform the rotation via SVD or
...
weighting influences the rotation but not the result space.  The stages
of weighted ordinations are weighting, rotation, and de-weighting.
...
\item Perform the spectral decomposition
  \begin{equation}
    -\tfrac{1}{2}\vec{M} = \vec{\tilde U} \bLambda \vec{\tilde U'}\;,
  \end{equation}
...
\index{principal coordinate analysis!weighted}
Function \icode{wPCoA} performs weighted centring followed by
\icode{eigen} and de-weighting.
...
\item Let \vec{Y} be the response matrix standardized to unit sum and
  ...
  \end{equation}
  This is  \indexrev{Chi-square}{transformation} because by usual
...
\index{correspondence analysis} \index{singular value decomposition}
Function \icode{CA} is based on \icode{svd} of transformed data.

<<echo=false>>=
dump("CA", file = "")
@
This returns a result from \icode{svd} with row and column scores (\vec{V} and
\vec{U}) and singular values which are square roots of eigenvalues.

\paragraph{Spectral Decomposition}
\label{sec:alg:CA:eig}
\index{correspondence analysis} \index{spectral decomposition}
...
\item The original transformation (\ref{eq:CAmattrans}) was not
  centred, and the first eigenvector of (\ref{eq:CAeigen}) will be
  so called trivial solution of $\lambda=1$ to do the centring omitted
  earlier. We wish to discard this first trivial axis.
\end{enumerate}
Similarly as in PCA, this method will return rubbish for the last axes if
the number of columns is larger than $n-1$. 

\index{correspondence analysis} \index{spectral decomposition}
Function \icode{CAeig} implements this using \icode{eigen}, and removing
the trivial axis.
<<echo=false>>=
dump("CAeig", file="")
@
...
\index{correspondence analysis!distance-based}
\index{principal coordinate analysis!weighted}
...
\item Let \vec{Y} be the response matrix standardized to unit sum,
  \vec{r} and \vec{c} its row and column sums with corresponding
...
\index{correspondence analysis!distance-based}
\index{principal coordinate analysis!weighted}
Function \icode{CAdist} transforms data and performs weighted
principal coordinate analysis with \icode{wPCoA}.
...
\index{reciprocal averaging} \index{correspondence analysis!reciprocal
  averaging} \index{weighted averages}
 The power method of PCA
(p.~\pageref{sec:alg:PCA:power}) can be also used to find the first axis
of CA. The only difference is that instead of weighted sums of PCA we use
...
averaging) was regarded as a simpler and more intuitive method than mathematically
complicated PCA. Similar simple power algorithm can be used for the
first axis of PCA (p.~\pageref{sec:alg:PCA:power}), but even in those
times, computer programs used more effective but mathematically more
complicated algorithms for PCA. Similarly as with PCA, the eigenvalue is the ratio
of change of scales between successive iterations,
and the iterations can be stopped when the ratio stabilized. In CA,
eigenvalue is always $\lambda_1 \le1$ so that the vectors shrink during
iterations.
...
\code{shrinkage} which  is the current estimate of
eigenvalue.

\index{reciprocal averaging} \index{correspondence analysis!reciprocal
  averaging} \index{weighted averages} 
Function \icode{CAwa1} only calls repeatedly Vegan \icode{wascores}.
<<echo=false>>=
dump("CAwa1", file="")
@

\subsubsection{Nonmetric Multidimensional Scaling (NMDS)}
\label{sec:alg:NMDS}

\index{nonmetric multidimensional scaling}
...
\citep{Krus64, Krus64num}. \index{nonmetric multidimensional scaling!stress}
...
units that share no species or have nothing in common. If we keep the fitted values tied, the
ordination tries to put all these extreme sites to the same distance
from each other, resulting in spherical or ring like
configuration. \citet{Krus64, Krus64num} recommends allowing breaking
ties.  He calls this primary tie treatment and such breakable ties as weak.
 Function \code{NMDS}
...
ties in an order dependent way. Vegan function \icode{monoMDS} (used
in \icode{metaMDS}) has alternative ways of tie treatment, but defaults to
primary treatment (weak ties). 

\index{nonmetric multidimensional scaling}
\index{nonmetric multidimensional scaling!stress}
...
parameters. There are $nk$ parameters for $n$ points on $k$
dimensions which makes NMDS a large problem. For effective optimization
...
stress with respect to coordinate $u_{ik}$ for point $i$ on dimension
$k$ \citep{Krus64num}:
\begin{equation}
 \frac{\D S}{\D u_{ik}} = S \sum_{j=1}^n \left[ \left( \frac{d_{ij} - \hat{d}_{ij}}{S^*} - \frac{d_{ij}}{T^*}\right) \frac{u_{ik} - u_{jk}}{d_{ij}} \right]\;.
\end{equation}
In addition, we want to have a robust and effective optimization
routine. \citet{Krus64num} described the method of steepest descent,
or always moving along the gradient, but this other methods could be used
\citep{Pres92, NoceWrig99}.  R function \code{optim} has several good
alternatives, and \code{NMDS} below defaults to limited-memory
Broyden-Fletcher-Goldfarb-Shanno (BFGS) method.  \icode{isoMDS}
function in the MASS package \citep{MASS} uses plain BFGS, and 
\icode{monoMDS} in vegan steepest descent.
...
\item Set starting values for iteration. The function uses random
  uniform coordinates if initial configuration (\code{u}) was not
...
\item Minimize the stress with \icode{optim}. Function allows
  ...
  from \icode{isoreg}, and \code{y} refers to ordination distances $d$
   and \code{yf} to $\hat d$.
\end{enumerate}

Function \icode{NMDS} has functions for \texttt{stress}, its derivative
\texttt{dstress} and a call to \icode{optim} for ordination.
<<echo=false>>=
dump("NMDS", file = "")
@
\index{nonmetric multidimensional scaling}
...
a similar method (except not handling ties and using BFGS optimization),
...
variables that restrict the ordination. The basic methods are
Redundancy Analysis (RDA) which is generalized from PCA,  Constrained
Correspondence Analysis (CCA) which is generalized from CA, and 
distance-based RDA generalized from Principal Coordinates Ordination.

In constrained ordination independent (environmental) variables are used as
explanatory variables in linear model, and ordination of the fitted values.
We could do the basic analysis with R functions \icode{lm} and \icode{svd}.

\begin{Schunk}
\begin{Sinput}
## assume to have dependent data dune and independent data dune.env
fv <- fitted(lm(as.matrix(dune) ~ ., data=dune.env)
svd(scale(fv, scale = FALSE))
\end{Sinput}
\end{Schunk}

In practice, we want to do more as we see below.
...
the method of constrained ordination. However, we get the same
results with  other algorithms. \citet{LegeLege98} showed that a
...
(p. \pageref{sec:alg:QR}) to estimate the fitted values and residual
...
\index{redundancy analysis}
The analysis of \vec{\hat Y} gives the constrained part of the analysis. The
...
Function \icode{RDA} is based on R functions \icode{qr} and \icode{svd}.
...
analysis with nteresting variables. This gives as the following
sequence of models:
\begin{eqnarray}
...
of constraints \vec{X} and conditions \vec{Z} together.

It is necessary to combine conditions \vec{X} and constraints \vec{Z}
into same model matrix. Although the direct effects of conditions were
removed in the first step, they are not orthogonal to \vec{X} and will
have interactions.
Alternatively, we can use  residuals of \vec{X} regressed on \vec{Z},
but the combined matrix \vec{[X,Z]} gives the same results.  The vegan
package uses the combined matrix, but CANOCO uses the regression
residuals approach \citep{CANOCO}.

SVD of \vec{\hat Y} gives the
constrained ordination, and the analysis of \vec{Y_r} gives the
residual ordination.

\index{redundancy analysis!partial}
Function \icode{pRDA}  allows omitting either conditions or
constraints or both so that the function can be used for pRDA, RDA,
pPCA (= PCA after removing background variables), or PCA. Vegan
function \icode{rda} has the same general design. \icode{pRDA} is based
on R functions \icode{qr} and \icode{svd}.

<<echo=false>>=
dump("pRDA", file="")
@
The alternative approach using regression residuals of constraints can
be achieved by making the following change in the code:
...
Analysis is an analogous generalization of RDA in the CA framework.  The
...
\item Perform Chi-square standardization of \vec{Y} matrix with
  (\ref{eq:CAstand}).
...
Function \icode{pCCA} is similar to \icode{pRDA}, but adds handling weights
and Chi-square standardization. Similarly as \texttt{pRDA}, it allows omitting
conditions \vec{Z} and constraints \vec{X}. Function is based on \icode{qr} and
\icode{svd}.
...
\paragraph{Alternating Weighted Averages and Linear Regression}
\label{sec:alg:WALC}

\Citet{terB86} originally presented CCA with an algorithm of
alternating weighted averages and linear regression, and
\citet{Palm93} explains this approach in a very educational way. The
mere weighted averages would give the usual CA with the Reciprocal
Averaging algoritm (p. \pageref{sec:alg:CA:RA}). This is not a
recommended algorithm, and it is not used in R functions for CCA. The
algorithm is presented only for educational purposes.

The following implementation is a minor variant of \code{CAwa1} code
of Reciprocal Averaging (p. \pageref{sec:alg:CA:RA}): the site scores
found as weighted averages of species scores are renamed to \code{w},
and the site scores \code{u} are found as fitted values of weighted
linear regression predicting scores \code{w} with model matrix of
constraining variables as independent variables and row sums of
dependent data as weights. Function \icode{CCAwa1} uses \icode{lm.wfit} for linear
regression instead of formula based full-blown linear regression of \texttt{lm}.

<<echo=false>>=
dump("CCAwa1", file="")
@

Row scores \code{w} are called WA scores, and row scores \code{u} are
called LC scores \citep{Palm93}. The current function deshrinks
species scores \code{v} and WA scores \code{w}, but not the fitted
values of linear regression \code{u}, and therefore the scale of the
scores shrinks during iteration. The function finds only one axis.

Similar addition of one line of linear regression and renaming of
scores \code{w} in Power algorithm in function \icode{PCApot1} for PCA
(p. \pageref{sec:alg:PCA:power}) will give RDA.  No weights are needed
in linear regression step in RDA.
...
is related to PCA \citep{LegeAnde99, dbrda}. We need to make the following
changes to RDA (cf. function \icode{RDA} against \icode{PCoA}):

\begin{enumerate}
\item	The dependent data will be Gower double-centred matrix $-\tfrac{1}{2} \vec{M}$ of
  eq. \ref{eq:gower-double} (p.~\pageref{eq:gower-double}).
\item Since dependent matrix is symmetric, functions \texttt{qr.fitted} and \texttt{qr.resid}
must be applied to both sides of matrix.
\item Spectral decomposition (\icode{eigen}) is used instead of \texttt{svd}.
\end{enumerate}

\index{redundancy analysis!distance-based}
Similarly as with PCoA, analysis will be identical to RDA if input distances are
Euclidean, but with non-Euclidean distances there may be negative eigenvalues.
They are rare in the constrained component, but they are regular in residual
components.

\index{redundancy analysis!distance-based}
Function \icode{dbRDA} performs first Gower-centring and the transformed matrix
and then its transposed is processed with \icode{qr} decomposition. The ordination step
is based on \icode{eigen}.
...
\index{redundancy analysis!distance-based}
There is no way of getting species
scores without original data matrix, because distance matrix has no
memory of species that were used in its calculation.

Function \icode{dbrda} in vegan follows this algorithm. Function \icode{adonis2}
is a short-cut to the same method but skips the ordination (spectral decomposition).

\index{redundancy analysis!distance-based}
There is an alternative simplified tool that is often confused with distance-based
RDA: the distance matrix is analysed with PCoA, and the vectors scaled by
eigenvalues are used as input in RDA. With non-Euclidean distances having
negative eigenvalues, only eigenvectors with above-zero eigenvalues will be
used. This discards the imaginary component of dissimilarities and analyses only
the Euclidified real component.
Function \icode{capscale} in vegan uses this algorithm.
...
\subsection{Reconstitution of Data}
\label{sec:alg:pred:data}
...
\end{equation}
which uses diagonal matrix of eigenvalues \vec{\bLambda} and orthonormal row and column
...
standardization of (\ref{eq:PCAstand}, p.~\pageref{eq:PCAstand}):
\begin{equation}
  y_{ij} = s_j \bar{y}_{ij} + z_j\;,
\end{equation}
and in CA we must undo (\ref{eq:CAstand}, p.~\pageref{eq:CAstand}):
\begin{equation}
...
using arbitrary dissimilarities, but we can  recover the
\citet{Gower66} matrix $-\tfrac{1}{2} \vec{M}$ derived from the original
dissimilarities, and from this we can recover the dissimilarities \vec{D}.
If $\vec{D}$ was based on non-Euclidean
...
estimate he distances estimated from
...
\begin{eqnarray}
  ...
  \lambda_k |  (u_{ik} - u_{jk})^2 \\
   d_{ij} &=& \sqrt{m^2_{ii} + m^2_{jj} - 2 m^2_{ij}}\;,
\end{eqnarray}
where $d$ are original dissimilarities
...
components (however, this does not work in distance-based RDA):
\begin{equation}

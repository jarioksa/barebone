...
reduced still further, and we speak of reduced or rank-deficit modes.
...
\index{singular value decomposition}
Function \icode{svd} performs the singular value decomposition. In
...
\index{QR decomposition}
QR decomposition is a useful tool to avoid matrix inversion and loss
of numerical precision in eq. \ref{eq:Yhat}. Any real matrix can be
decomposed as $\vec{X} = \vec{QR}$, where $\vec{R}$ is an upper (or
right) triangular matrix, with possible extra lines filled with zeros,
and $\vec{Q}$ is an orthogonal matrix (again last lines filled with
zeros if number of rows is larger than number of
columns). Orthogonality means that the transpose of $\vec{Q}$ is its
inverse $\vec{Q'} = \vec{Q}^{-1}$ and $\vec{Q'Q} = \vec{I}$.

\index{QR decomposition}
Let us write eq. \ref{eq:Yhat} with QR decomposition of $\vec{X}$:
\begin{eqnarray}
 \vec{\hat Y} &=& \vec{QR}(\vec{R'Q'QR})^{-1}\vec{R'Q'Y} \\
  \label{eq:QQtY}
&=& \vec{QR} \vec{R'}^{-1}\vec{R}^{-1} \vec{R'Q'} \\
&=& \vec{QQ'Y}\;.
\end{eqnarray}
...
numerical stability. \index{aliased variables} The R implementation keeps the order of explanatory
...
The use of \texttt{qr} function is very simple. After making the QR decomposition
...
useful intuition of the data. It is best not to look at them. Actually, \texttt{qr} function
...
Principal component analysis, Principal coordinate analysis and correspondence analysis are the major methods discussed in this chapter. These all are basically so similar that current vegan
...
eigenvalues can be defined, and the rest are rubbish. The following function will
...
iteratios when the eigenvalue stabilized in iterations. The following
...
\item \icode{princomp} uses eigen decomposition of covariance or
  ...
  \texttt{loadings}.
\item \icode{pca} and icode{rda} in vegan package uses SVD in its PCA. The function
...
the number of columns is larger than $n-1$. Direct implementation in R is:
<<echo=false>>=
dump("CAeig", file="")
@
...
\index{reciprocal averaging} \index{correspondence analysis!reciprocal
  averaging} \index{weighted averages} The power method of PCA
...
averaging) was regarded as a simpler method than mathematically more
complicated PCA. Similar simple power algorithm can be used for the
first axis of PCA (p.~\pageref{sec:alg:PCA:power}), but even in that
time, computer programs used more effective but mathematically more
complicated algorithms. Similary as with PCA, the eigenvalue is he ratio
...
\code{shinkage} which  is the current estimate of
...
ties.  He calls this primary tie treatment and such breakable ties as weak. Function \code{NMDS}
...
for difficult tasks like NMDS and other methods could be used
...
removed in the first step, they are not orthogonal to \vec{X}.
...
function \icode{rda} has the same general design.
...
Analysis is an analogous generalization of RDA in the CA framwork.  The
...
dependent data as weights. The function uses \icode{lm.wfit} for linear
...
scores \code{w} in Power algorithm of PCA

...
analysing the interesting variables. This gives as the following
sequence of models:
\begin{eqnarray}
...
Analysis is a parallel generalization of RDA. CA is weighted PCA with
Chi-square metric, and we have to deal with weighting and
deweighting. The following overview of the algorithm only discusses

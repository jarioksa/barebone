...
Nonmetric multidimensional scaling (NMDS) is only defined by monotone
regression and goodness of fit measure \citep{Krus64, Krus64num}. The
goodness of fit measure is called stress $S$ and it is defined as
\begin{equation}
...
$\hat d$ are the fitted distances. The monotone regression states that
$\hat d$ have the same rank order as observed dissimilarities
\citep{Krus64, Krus64num}.

Only monotone regression and stress were defined, but not the way how
to minimize the function which makes NMDS strikingly different from
eigenvector methods. NMDS is a nonlinear optimization problem. For $n$
points and $k$ dimensions we must estimate $nk$ parameters which makes
NMDS a hard problem. We need information on the gradient of stress
with respect tot he location of the points, and we need an effective
optimization routine.

The derivative of the stress, or its gradient, is defined as
\citep{Krus64num}:
\begin{equation}
  \frac{\D S}{\D x_{il}} = S \sum_{j=1}^n \left[ \left( \frac{d_{ij} - \hat{d}_{ij}}{S^*} - \frac{d_{ij}}{T^*}\right) \frac{x_{il} - x_{jl}}{d_{ij}} \right]\;,
\end{equation}
where $x$ is the coordinate of point $i$ on dimension $l$. In
...
for difficult tasks like NMDS \citep{Pres92}.  R function
\icode{optim} has several good alternatives, and \citet{MASS} use
Broyden-Fletcher-Goldfarb-Shanno algorithm in their \icode{isoMDS}
function in the MASS package.

The following implementation only has function for \code{stress} and
its derivative \code{dstress}.  The only other steps are that the
function sets random initial configuration if none was provided
(argument \code{x}) and calls \icode{optim} to find the configuration
that minimizes the stress in \code{k} dimension using the given
\code{method}.
...
the same method, but implemented in C.

...
Nonmetric multidimensional scaling (NMDS) is defined by monotone
...
appear in the equation for stress.  They influence the monotone
regression that states that $\hat d$ have the same rank order as
observed dissimilarities \citep{Krus64, Krus64num}. The monotone
regression is implemented in R function \icode{isoreg}.
...
\end{equation}
\citet{Krus64num} described the method of steepest descent, or always
moving along the gradient, but this is usually ineffective for
difficult tasks like NMDS \citep{Pres92}.  R function \icode{optim}
has several good alternatives, and \citet{MASS} use
...
contain the final configuration in matrix form and \code{value} the
final stress. Function \icode{isoMDS} in the MASS package \citep{MASS}
uses the same method, but implemented in C.

...
analysis. The standard R function use the same algorithms, but they
also have commands to check the input, handle common error situations
and exception and to process the results. All these features make the
functions usable, but they hide the plain core of the methods. This
appendix has nothing of that user friendly interface, but the
functions expect the input be correct, and the user to handle the
results. The purpose is only to show the minimal code of translating
matrix algebra into R code, and help the user to find the core
operations in real R functions.

\section{Matrix Algebra in R}

R is not really designed primarily for matrix algebra. For instance,
...
squares is 1, or $\vec{U'U} = \vec{I}$ and $\vec{V'V} =
\vec{I}$. The importance of axes is conveyed in singular
values. The singular values are ordered by their magnitude, and for an
$n \times m$ matrix there may be $r = \min(n, m)$ above-zero singular
values. The number of the above zero singular values gives the rank of
the matrix, or the number of independent
...
centred data, the rank is reduced by one because the mean is
fixed. If some of the variables can be expressed as linear
combinations of other variables, the rank is reduced still further,
...
Spectral decomposition, also known as eigenvector decomposition, is
used for a symmetric matrix. Ecological data matrices are
nonsymmetric, and spectral decomposition is used for their
...
somewhat lighter and easier to implement, and originally earlier was the
...
ordination we only use centred crossproducts. This also
...
ordination. We can use (\ref{eq:databack}) to get values in the
original scale. Package vegan implement such procedures in
\texttt{fitted} and \texttt{residuals} functions. In the following model
...
components:
\begin{equation}
  \vec{Y} = \vec{U} \bLambda^{1/2} \vec{V'} + \vec{U_r} \vec{\bLambda_r}^{1/2} \vec{V'_r}\;,
\end{equation}
where the terms indexed with \vec{r} refer to unconstrained residual
...
``residual lengths'', but these do not estimate precisely the original
data.
...
functions are:
\begin{align}
 \vec{V} &= \vec{\bar Y'} \vec{U} \vec{\bLambda}^{-1/2}\\
 \vec{U} &=  \vec{\bar Y} \vec{V} \vec{\bLambda}^{-1/2}
\end{align}
...
diagonal matrices $\vec{R} = \mathrm{diag}(\vec{r})$ and $\vec{C} =
\mathrm{diag}(\vec{c})$:
\begin{align}
\vec{V} &= \vec{C^{-1/2}} \vec{\bar Y'} \vec{R^{1/2}}
\vec{U} \vec{\bLambda^{-1/2}}\\
\vec{U} &= \vec{R^{-1/2}} \vec{\bar Y} \vec{C^{1/2}}
\vec{V} \vec{\bLambda^{-1/2}}
\end{align}
Supplying new \vec{Y} allows finding scores for new rows or
columns. The needed weights are also found from the supplied \bm{X}.
...
\begin{align}
 \vec{U} &= \vec{\bar X B}\\
 \vec{V} &= \vec{C^{-1/2} \vec{\bar Y'} \vec{R^{1/2}}
 \vec{U} \vec{b\Lambda^{-1/2}\\
 \vec{W} &= \vec{R^{-1/2}} \vec{\bar Y} \vec{C^{1/2}}
 \vec{V} \vec{\bLambda^{-1/2}}
\end{align}
...
\vec{U}, and new dependent data for WA scores \vec{W} or species scores
\vec{V}.
...
\bm{U}, and you can repeat this cyclically. However, the scores of
